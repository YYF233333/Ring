...
@onready var piece_scene = preload(Shared.TETROMINO_PATH + "Scenes/piece.tscn")
@onready var ghost_tetromino_scene = preload(Shared.TETROMINO_PATH + "Scenes/ghost_tetromino.tscn")
...
func _ready():
	...
	if is_next_piece == false:
		...
		wall_kicks = Shared.wall_kicks_i if tetromino_data.tetromino_type == Shared.TetrominoType.I else Shared.wall_kicks_jlostz
		ghost_tetromino = ghost_tetromino_scene.instantiate() as GhostTetromino
		ghost_tetromino.tetromino_data = tetromino_data
		self.get_parent().get_parent().add_child.call_deferred(ghost_tetromino)
...
func rotate_tetromino(direction: int):
	...
	if tetromino_data.tetromino_type == Shared.TetrominoType.O:
		return
